...
export async function POST(request: NextRequest) {
  try {
    ...
    // 根据不同模型设置不同的字符限制 - 修复计算错误
    let reasonableMaxChars
    if (selectedModel.includes('sonoma')) {
      reasonableMaxChars = 800000 // Sonoma模型：80万字符 (降低以确保稳定)
    } else if (selectedModel.includes('gemini')) {
      reasonableMaxChars = 600000  // Gemini模型：60万字符 (降低以确保稳定)
    } else {
      reasonableMaxChars = 300000  // 其他模型：30万字符 (降低以确保稳定)
    }

    // 修复截断逻辑 - 确保正确处理长文本
    const shouldTruncate = estimatedTokens > maxInputTokens || content.length > reasonableMaxChars
    ...
    if (shouldTruncate) {
      ...
      // 直接从开头截断，不使用智能采样
      processedContent = content.substring(0, finalMaxChars)
      wasTruncated = true
...
5. ${wasTruncated ? '内容已从开头截断，请重点分析前半部分出现的主要角色和场景' : '请优先分析主要角色和场景'}`,
    ...
    if (!response.ok) {
      ...
      if (wasTruncated || content.length > 20000) {
        throw new Error(`长文本处理失败 (${content.length.toLocaleString()}字符)。\n\n${errorMessage}\n\n建议：\n1) 将文档分段处理\n2) 使用更短的文本\n3) 稍后重试\n4) 检查API密钥配置`)
      }
