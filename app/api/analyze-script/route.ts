...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 模型配置
    const modelConfigs = {
      'openrouter/sonoma-dusk-alpha': { name: 'Sonoma Dusk Alpha', contextLength: 2000000 },
      'openrouter/google/gemini-1.5-pro': { name: 'Gemini 1.5 Pro', contextLength: 1000000 },
      'openrouter/anthropic/claude-3.5-sonnet': { name: 'Claude 3.5 Sonnet', contextLength: 200000 }
    }

    // 确保有有效的模型选择 - 强制使用默认模型
    const finalSelectedModel = (modelParam && modelParam.trim()) ? modelParam : 'openrouter/sonoma-dusk-alpha'
    const modelConfig = modelConfigs[finalSelectedModel as keyof typeof modelConfigs] || modelConfigs['openrouter/sonoma-dusk-alpha']
